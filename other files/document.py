...
class Document:
	...
	def __init__(self,docs,num_clu):
		...
		#print self.sent_similarity[len(self.sent_similarity)-1]
	...
	def select_cluster_sentences(self):
		...
		for clust in self.clusters:
			...
			for i in range(len(clust)):
				val = 0.3*sent_weight_sum[i] + 0.7*sent_edgewt_neighbours[i] + 0.0*sent_inv_nowords[i]
	...
	def printinit_clust(self):
		for i in range(len(self.clusters)):
			print self.sentences[self.clusters[i][0]]

		print ""

	def printmedoid_sentences(self):
